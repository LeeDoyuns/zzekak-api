...
import l1a.jjakkak.infra.domain.address.model.PathFindRequest
...
internal class ConnectionModule(val confmKey: String) {
    ...
    // 길찾기
    fun findPath(strtLocX: String, strtLocY: String, endLocX: String, endLocY: String, appointmentTime: ZonedDateTime): SearchedPathResponse {
        ...
        var req: JsonObject = PathFindRequest(startCoordinate, destinationCoordinate, confmKey, arrivalTime).returnToJSON()
        var response = connectionFindPath(pathFindUrl, req)
        val pf = PathFindResponse.to(response)

        val result = SearchedPathResponse.create(
